...
if [[ "$OS" == "Darwin" ]]; then
  ...
  # Covers native Linux and WSL (Windows Subsystem for Linux) on Ubuntu/Debian.
  if ! command -v apt-get >/dev/null 2>&1; then
    fail "This script's Linux path only supports apt-based distros (Ubuntu/Debian/WSL). Install python3, python3-venv, cmake, build-essential, libopenblas-dev, liblapack-dev manually, then run: pip3 install -r requirements.txt"
  fi
  log "Installing system packages via apt (will prompt for your sudo password)"
  sudo apt-get update || fail "apt-get update failed"
  sudo apt-get install -y python3 python3-venv python3-pip cmake build-essential libopenblas-dev liblapack-dev \
    || fail "apt-get install failed"

else
  fail "Unrecognized OS ($OS). This script supports macOS and Linux/WSL. On Windows, use WSL (see facerecog/WINDOWS.md) or run setup.ps1 for a native install."
fi
