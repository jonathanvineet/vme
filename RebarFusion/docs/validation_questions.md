...
**Confidence revised down** (`docs/research/apollo_registration_audit.md`): the "shared coordinate frame" claim this question rested on does not hold precisely — same-labeled reference symbols on this exact sheet pair disagree by 62-421mm with no consistent transform. The 11mm N7/T8@200 co-location is still the best available evidence, but it sits inside that same noise band, so it is **plausible, weakly corroborated** rather than a near-certain match. Still worth an engineer's five minutes; just don't treat a "no" as surprising.
