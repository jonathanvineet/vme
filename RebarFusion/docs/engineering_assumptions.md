...
| # | Assumption | Evidence | Enforced in | Confidence |
|---|---|---|---|---|
| A12 | Views cross-reference **symbolically** (numbered section markers, detail callouts, shared reference codes), not spatially. A section is routinely drawn in unrelated sheet space; no coordinate transform between views should be assumed. **Weak exception, quantified**: a single-view mould sheet (`M`/`M1`) and its `(R)` counterpart's first drawn view sometimes sit in coincidental proximity (bounding-box overlap 43-73% in 2/2 measured cases) — but same-labeled reference symbols within that overlap still disagree by 62-421mm with no consistent transform, and the schedule-bearing mould sheet (`M2`) shares zero overlap with `(R)` (2/2 measured). Proximity is at most weak corroboration, never a registration to build on. | `PW-GF-09(M1)` Section 1 placed in blank space; section-marker symbols 1/2/3 on both PW and SS sheets; full quantitative audit in `docs/research/apollo_registration_audit.md` (5 sheet-pairs, one project). | Phase 12.2: spatial distance is `polarity=UNKNOWN`, never a qualifying signal | High |
