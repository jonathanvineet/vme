...
## Phase 12.4 — The one real PhysicalIdentity needs engineering verification (A17; evidence sharpened by Phase 13.2)
The single ACCEPTED identity merges two `T16` observations from `PW-GF-09(R).dwg`. Phase 13.2's investigation found: both observations are 22mm section-view cross-section symbols (not drawn bars), ~570mm apart, **each with its own `2 -T16` annotation**, among 8 such annotations in two sheet regions — evidence *leaning toward two distinct physical bar groups* (i.e., a false merge), but the alternative (one pair of edge bars labeled at both faces of a section) cannot be excluded without engineer-level reading of the section geometry. **Open; precisely framed for engineer review** in `benchmark/corpus/Apollo/ground_truth/notes.md`. Separately: all cross-drawing `SS-GF-01(M)`↔`(R)` pairs land in `REVIEW` because N-code → T-mark resolution requires the schedule/annotation evidence generators that don't exist yet — designed behavior, now also measured: the Apollo baseline is precision/recall 0.0 with 1196/1198 pairs at REVIEW, the number future phases must beat.
